function buildExternalAnchors() {
    // Append 'external' icon to <a> with target external
    const anchors = document.querySelectorAll('a[target="_blank"]');
    for (const anchor of anchors) {
        anchor.innerHTML += ' <i class="fas fa-external-link-alt small"></i>';
    }
}
...
function buildScrollTopLinks() {
    const scrollTopLinks = document.getElementsByClassName('scroll-top');
    for (const link of scrollTopLinks) {
